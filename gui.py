            ...
            if not skip_group_update:
                ...
                for (gn, sh), g in ug.items():
                    ...
                    if r:
                        gid_map[(gn, sh)] = r[0]
                for g in data.get('groups', []):
                    gid = gid_map.get((g.get('name'), g.get('sheet_name', 'Main')))
                    if gid:
...
class App(QMainWindow):
    ...
    def rename_group_dialog(self, g):
        ...
        if ok and nm.strip():
            ...
            # Update in-memory immediately BEFORE async save
            g.group_name = nm
            g.update()
            
            # Mark as dirty so reload doesn't create duplicate
            self._group_renames[g.sheet_name] = {**self._group_renames.get(g.sheet_name, {}), old: nm}
            self._ignore_db_reload_until = time.time() + 3.0
            
            # Update database
            try:
                self._ensure_db_conn()
                self.db_cursor.execute("UPDATE node_groups SET name=%s WHERE name=%s AND sheet_name=%s",
                                    (nm, old, g.sheet_name))
                self.db_conn.commit()
            except Exception as e:
    ...
    def delete_group_and_nodes(self, g):
        ...
        members = [n.node_name for n in nodes_to_del]
        
        # 1. UI Scene aur App Memory se Nodes ko completely remove karein
        ...
        # Extra Safety: Pure app ke nodes backup array me se bhi connections clear karein
        # Taki koi naya link ya orphan entry bachi na reh jaye
        for remaining_node in self.nodes:
            # Agar kisi bache hue node ke connections me delete hone wale nodes hain, toh unhe saaf karein
            if hasattr(remaining_node, 'connections'):
                remaining_node.connections = [c for c in remaining_node.connections if c not in members]

        # 2. UI Scene aur App Memory se Group ko remove karein
        if g.scene():
            self.scene.removeItem(g)
        if g in self.groups:
            self.groups.remove(g)
        ...
        try:
            ...
            # Pehle junctions/foreign keys handle karne ke liye connections explicitly uda dete hain
            if members:
                ph = ",".join(["%s"] * len(members))
                # Delete incoming and outgoing connections from DB to avoid foreign key or ghost connections
                self.db_cursor.execute(f"DELETE FROM connections WHERE node_id IN (SELECT id FROM nodes WHERE name IN ({ph}))", members)
                self.db_cursor.execute(f"DELETE FROM connections WHERE target_node_id IN (SELECT id FROM nodes WHERE name IN ({ph}))", members)
                
                # Ab actual nodes table se delete karein
                self.db_cursor.execute(f"DELETE FROM nodes WHERE name IN ({ph})", members)
                
            # Ab group table se delete karein
            self.db_cursor.execute("DELETE FROM node_groups WHERE name=%s AND sheet_name=%s", (gn, sh))
            self.db_conn.commit()
        except Exception as e:
        ...
        self.log_audit("Group & Nodes Deleted", f"Group='{gn}' | Sheet='{sh}' | DeletedNodes=[{', '.join(members)}]")
        
        # 4. Immediate Save Trigger - network_data.json aur DB state ko bilkul sync karega
        self.save_data()
        self.log_panel.add_log(f"Deleted group '{gn}' and its {len(members)} nodes")
        
        # 5. Left Panel/UI lists ko forced refresh karein
        if hasattr(self, 'node_list_panel'):
            self.node_list_panel.refresh(self.nodes, self.groups, self.current_sheet)
