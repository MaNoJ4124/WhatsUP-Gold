...
# ═══════════════════════════════════════════════════════════════════════════════
# UNDO / REDO — Command Pattern (Priority 1)
# ═══════════════════════════════════════════════════════════════════════════════
class Command:
    """Base command for undo/redo stack."""
    def execute(self): pass
    def undo(self): pass
    def description(self): return "Command"
...
class UndoStack:
    ...
    def can_redo(self): return self._idx < len(self._stack) - 1
    def clear(self): self._stack.clear(); self._idx = -1

# ─── Concrete Commands ───────────────────────────────────────────────────────
class MoveNodeCommand(Command):
    def __init__(self, app, node, old_pos, new_pos):
        self.app = app; self.node = node
        self.old_pos = old_pos; self.new_pos = new_pos
    def execute(self):
        self.node.setPos(self.new_pos)
        self.node.base_x = self.new_pos.x(); self.node.base_y = self.new_pos.y()
        self.app.update_connections_for(self.node)
    def undo(self):
        self.node.setPos(self.old_pos)
        self.node.base_x = self.old_pos.x(); self.node.base_y = self.old_pos.y()
        self.app.update_connections_for(self.node)
    def description(self): return f"Move '{self.node.node_name}'"

class AddNodeCommand(Command):
    def __init__(self, app, node):
        self.app = app; self.node = node
    def execute(self):
        if self.node not in self.app.nodes:
            self.app.nodes.append(self.node)
        if self.node.sheet_name == self.app.current_sheet and not self.node.scene():
            self.app.scene.addItem(self.node)
        self.app._update_dashboard()
    def undo(self):
        if self.node.scene():
            self.app.scene.removeItem(self.node)
        if self.node in self.app.nodes:
            self.app.nodes.remove(self.node)
        self.app._update_dashboard()
    def description(self): return f"Add '{self.node.node_name}'"

class DeleteNodeCommand(Command):
    def __init__(self, app, node, group_name=None):
        self.app = app; self.node = node
        self.group_name = group_name
        self.conn_names = [c.node_name for c in node.connections]
    def execute(self):
        if self.node.group:
            self.node.group.remove_node(self.node)
        for c in list(self.node.connections):
            if self.node in c.connections:
                c.connections.remove(self.node)
        key_set = [k for k in self.app.connection_lines if self.node.node_name in {n for n in k}]
        for k in key_set:
            line = self.app.connection_lines.pop(k, None)
    ...
    def undo(self):
        self.app.nodes.append(self.node)
        if self.node.sheet_name == self.app.current_sheet:
            self.app.scene.addItem(self.node)
        if self.group_name:
            g = next((g for g in self.app.groups if g.group_name == self.group_name and g.sheet_name == self.node.sheet_name), None)
            if g:
                g.add_node(self.node)
        # Restore connections best-effort
        for cn in self.conn_names:
            tgt = next((n for n in self.app.nodes if n.node_name == cn), None)
            if tgt and tgt not in self.node.connections:
                self.node.connections.append(tgt)
                if self.node not in tgt.connections:
                    tgt.connections.append(self.node)
        self.app._update_dashboard()
    def description(self): return f"Delete '{self.node.node_name}'"

class RenameNodeCommand(Command):
    def __init__(self, app, node, old_name, new_name):
        self.app = app; self.node = node
        self.old_name = old_name; self.new_name = new_name
    def execute(self):
        self.node.prepareGeometryChange()
        self.node.node_name = self.new_name
        self.node.update()
    def undo(self):
        self.node.prepareGeometryChange()
        self.node.node_name = self.old_name
        self.node.update()
    def description(self): return f"Rename '{self.old_name}'→'{self.new_name}'"

class GroupColorCommand(Command):
    def __init__(self, app, group, old_color, new_color):
        self.app = app; self.group = group
        self.old_color = old_color; self.new_color = new_color
    def execute(self):
        self.group.custom_color = self.new_color; self.group.update()
    def undo(self):
        self.group.custom_color = self.old_color; self.group.update()
    def description(self): return f"Color '{self.group.group_name}'"
...
def embedded_create_tables_if_not_exist():
    ...
    try:
        ...
        try:
            cur.execute("ALTER TABLE nodes ADD COLUMN ping_interval INT DEFAULT 0")
        except Exception:
            pass
        # Priority 2: Group custom color
        try:
            cur.execute("ALTER TABLE node_groups ADD COLUMN custom_color VARCHAR(32) DEFAULT ''")
        except Exception:
            pass
        conn.commit()
        return True
        ...
        try:
            ...
            for n in data.get('nodes', []):
                ...
                cursor.execute("""INSERT INTO nodes(name,ip1,ip2,shape,sheet_name,base_x,base_y,size,starttime,total_down_time,last_active,color,alive,previous_alive,last_status_change,disabled,last_down_time,notes,position_updated_at,last_moved_by,tags,ping_interval) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE ip1=VALUES(ip1),ip2=VALUES(ip2),shape=VALUES(shape),sheet_name=VALUES(sheet_name),size=VALUES(size),starttime=VALUES(starttime),total_down_time=VALUES(total_down_time),last_active=VALUES(last_active),color=VALUES(color),alive=VALUES(alive),previous_alive=VALUES(previous_alive),last_status_change=VALUES(last_status_change),disabled=VALUES(disabled),last_down_time=VALUES(last_down_time),notes=VALUES(notes),tags=VALUES(tags),ping_interval=VALUES(ping_interval),base_x=IF(VALUES(position_updated_at)>=COALESCE(position_updated_at,0),VALUES(base_x),base_x),base_y=IF(VALUES(position_updated_at)>=COALESCE(position_updated_at,0),VALUES(base_y),base_y),position_updated_at=IF(VALUES(position_updated_at)>=COALESCE(position_updated_at,0),VALUES(position_updated_at),position_updated_at),last_moved_by=IF(VALUES(position_updated_at)>=COALESCE(position_updated_at,0),VALUES(last_moved_by),last_moved_by)""",
                               (n.get('name'), n.get('ip1', ''), n.get('ip2', ''), n.get('shape', 'circle'),
                                ...
                                n.get('tags', ''), int(n.get('ping_interval', 0))))
            ...
            if not skip_group_update:
                ...
                for (gn, sh), g in ug.items():
                    cc = g.get('custom_color', '') or ''
                    cursor.execute(
                        "INSERT INTO node_groups(name,sheet_name,custom_color) VALUES(%s,%s,%s) ON DUPLICATE KEY UPDATE sheet_name=VALUES(sheet_name),custom_color=VALUES(custom_color)",
                        (gn, sh, cc))
...
def embedded_load_network_from_mysql():
    ...
    try:
        ...
        if not sheets:
            sheets = ['Main']
        cur.execute(
            "SELECT id,name,ip1,ip2,shape,sheet_name,base_x,base_y,size,starttime,total_down_time,last_active,color,alive,previous_alive,last_status_change,disabled,last_down_time,notes,COALESCE(position_updated_at,0) AS position_updated_at,last_moved_by,COALESCE(tags,'') AS tags,COALESCE(ping_interval,0) AS ping_interval FROM nodes")
        nodes = []
        nbi = {}
        nbn = {}
        for r in cur.fetchall():
            nd = {'name': r['name'], 'ip1': r['ip1'] or '', 'ip2': r['ip2'] or '', 'shape': r['shape'] or 'circle',
                  ...
                  'tags': r.get('tags', '') or '',
                  'ping_interval': int(r.get('ping_interval', 0) or 0)}
            nodes.append(nd)
            nbi[r['id']] = nd
            nbn[r['name']] = nd
        cur.execute(
        ...
        cur.execute("SELECT id,name,sheet_name,COALESCE(custom_color,'') AS custom_color FROM node_groups")
        groups = []
        gbi = {}
        for gr in cur.fetchall():
            gd = {'name': gr['name'], 'sheet_name': gr['sheet_name'] or 'Main', 'nodes': [], 'custom_color': gr.get('custom_color', '') or ''}
            groups.append(gd)
            gbi[gr['id']] = gd
...
class NodeItem(QGraphicsItem):
    def __init__(self, nd: dict, app: "App"):
        ...
        self.tags: str = nd.get('tags', '') or ''          # comma-sep e.g. "#critical,#tower"
        self.ping_interval: int = int(nd.get('ping_interval', 0) or 0)  # 0=global
    ...
    def mouseReleaseEvent(self, e):
        if e.button() == Qt.LeftButton:
            ...
            if moved_dist > 5:
                ...
                # Priority 1: push to undo stack
                try:
                    cmd = MoveNodeCommand(self.app, self,
                                         QPointF(old_x, old_y), QPointF(new_x, new_y))
                    self.app._undo_stack.push(cmd)
                except Exception:
                    pass
    ...
    def to_dict(self):
        return {'name': self.node_name, 'ip1': self.ip1, 'ip2': self.ip2, 'shape': self.shape_type,
                ...
                'tags': self.tags, 'ping_interval': self.ping_interval,
...
class ConnectionLine(QGraphicsLineItem):
    """Priority 2: Health-encoded connection line color.
    green = both up, orange = one down, red = both down, dashed = one disabled."""
    ...
    def refresh_health(self):
        ...
        if not s or not t:
            return
        s_dis = getattr(s, 'disabled', False)
        t_dis = getattr(t, 'disabled', False)
        s_up  = getattr(s, 'alive', False) and not s_dis
        t_up  = getattr(t, 'alive', False) and not t_dis
        if s_dis or t_dis:
            style = Qt.DashLine
            color = _get_color("CDIS")
        elif s_up and t_up:
            style = Qt.SolidLine
            color = _get_color("CUP")       # green
        elif not s_up and not t_up:
            style = Qt.SolidLine
            color = _get_color("CDN")       # red
        else:
            style = Qt.SolidLine
            color = "#f39c12"               # orange — one side down
        self.setPen(QPen(QColor(color), 2.5, style, Qt.RoundCap))
...
class GroupItem(QGraphicsItem):
    def __init__(self, gd: dict, app: "App"):
        ...
        self.custom_color: str = gd.get('custom_color', '') or ''  # Priority 2: group color theme
    ...
    def paint(self, painter, opt, w=None):
        if not self.member_nodes:
            return
        painter.setRenderHint(QPainter.Antialiasing)
        # Priority 2: use custom color if set, else default
        if self.custom_color:
            bg = QColor(self.custom_color)
            bg.setAlpha(60)
            border_color = self.custom_color
        else:
            bg = QColor(_get_color("CGBG"))
            bg.setAlpha(100)
            border_color = _get_color("CGBD")
        painter.setPen(QPen(QColor(border_color), 1.5, Qt.DashLine))
        painter.setBrush(QBrush(bg))
        painter.drawRoundedRect(self._rect, 10, 10)
        painter.setPen(QColor(self.custom_color if self.custom_color else _get_color("GROUP_TEXT")))
    ...
    def contextMenuEvent(self, e):
        ...
        menu.addAction("🎨 Set Color", lambda: self.app.set_group_color(self))
...
class NodeListPanel(QFrame):
    def __init__(self, app, parent=None):
        ...
        # Priority 1: Tag filter bar
        self._tag_filter = QLineEdit()
        self._tag_filter.setPlaceholderText("🏷️  Filter by tag (#critical...)")
        self._tag_filter.textChanged.connect(lambda: self.refresh(self.app.nodes, self.app.groups, self.app.current_sheet))
        lo.addWidget(self._tag_filter)
    ...
    def refresh(self, nodes, groups, cs):
        ...
        # Priority 1: tag filter
        tag_q = getattr(self, '_tag_filter', None)
        tag_text = (tag_q.text().strip().lower() if tag_q else '')
        if tag_text:
            sn = [n for n in sn if tag_text in getattr(n, 'tags', '').lower()]
        gd = {}
        ug = []
        for n in sn:
            if n.group:
...
class EditNodeDialog(QDialog):
    def __init__(self, node: NodeItem, app: "App", parent=None):
        ...
        # Priority 1: Tags field
        tag_lbl = QLabel("Tags (comma-sep, e.g. #critical,#tower):")
        tag_lbl.setStyleSheet("font-weight:bold;font-size:11px")
        form.addRow(tag_lbl)
        ...
        form.addRow(size_lbl)
        self.size_slider = QSlider(Qt.Horizontal)
        self.size_slider.setRange(10, 80)
        self.size_slider.setValue(node.node_size)
        self.size_slider.setTickInterval(5)
        self.size_slider.setTickPosition(QSlider.TicksBelow)
        self.size_slider.valueChanged.connect(lambda v: self._size_lbl.setText(f"Node Size: {v}"))
...
class App(QMainWindow):
    ...
    def _build_ui(self):
        ...
        fm.addSeparator()
        fm.addAction("🖼️ Export PNG", self.export_png)
        fm.addSeparator()
        fm.addAction("Exit", self.close)
        em = mb.addMenu("Edit")
        undo_act = em.addAction("↩️ Undo", self._do_undo)
        undo_act.setShortcut("Ctrl+Z")
        redo_act = em.addAction("↪️ Redo", self._do_redo)
        redo_act.setShortcut("Ctrl+Y")
        em.addSeparator()
        add_act = em.addAction("➕ Add Node", lambda: self.add_node())
        add_act.setShortcut("Ctrl+N")
        grp_act = em.addAction("📦 Create Group", self.create_group)
        grp_act.setShortcut("Ctrl+G")
        search_act = em.addAction("🔍 Search", lambda: self.node_list_panel.search_input.setFocus())
        search_act.setShortcut("Ctrl+F")
        em.addSeparator()
        em.addAction("⌨️ Keyboard Shortcuts", self._show_shortcuts)
        vm = mb.addMenu("View")
        ...
        vm.addSeparator()
        vm.addAction("📊 Auto Layout", self._auto_layout)
    ...
    def _on_status_loaded(self, sd):
        ...
        try:
            ...
            if changed:
                ...
                # Priority 2: refresh connection line health colors
       ...
       #print(len(file_groups))
        ...
        for gd in file_groups:
            ...
            if existing_g:
                ...
                gi.custom_color = gd.get('custom_color', gi.custom_color) or gi.custom_color
            else:
    ...
    def _toggle_theme(self):
        ...
        # Update connection line colors (Priority 2: use health color)
        for line in self.connection_lines.values():
            try:
                line.refresh_health()
            except Exception:
    ...
    def _auto_layout(self):
        sn = [n for n in self.nodes if n.sheet_name == self.current_sheet]
        if not sn:
            QMessageBox.information(self, "Auto Layout", "No nodes on current sheet.")
            return
        if QMessageBox.question(self, "Auto Layout",
                                "Arrange nodes with force-directed layout?\n(Positions will be saved)",
                                QMessageBox.Yes | QMessageBox.No) != QMessageBox.Yes:
            return
        # Simple grid-based spring layout
        import math
        n = len(sn)
        ...
        for i, node in enumerate(sn):
            ...
            node._position_updated_at = time.time()
        for n in sn:
            self.update_connections_for(n)
        self.save_data()
        self.log_panel.add_log(f"Auto-layout applied to {len(sn)} nodes")

    # ══════════════════════════════════════════════════════════════════
    # PNG EXPORT (Priority 4)
    # ══════════════════════════════════════════════════════════════════
    def export_png(self):
        fn, _ = QFileDialog.getSaveFileName(self, "Export PNG", "", "PNG Image (*.png)")
        if not fn:
            return
        try:
            rect = self.scene.itemsBoundingRect().adjusted(-20, -20, 20, 20)
            pm = QPixmap(int(rect.width()), int(rect.height()))
            pm.fill(QColor(_get_color("CBG")))
            p = QPainter(pm)
            self.scene.render(p, source=rect)
            p.end()
            pm.save(fn)
            self.log_panel.add_log(f"PNG exported → {fn}")
            QMessageBox.information(self, "Success", f"PNG saved!\n{fn}")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"PNG export failed: {e}")

    # ══════════════════════════════════════════════════════════════════
    # KEYBOARD SHORTCUTS PANEL (Priority 3)
    # ══════════════════════════════════════════════════════════════════
    def _show_shortcuts(self):
        d = QDialog(self)
        d.setWindowTitle("Keyboard Shortcuts")
        d.setMinimumSize(380, 360)
        d.setStyleSheet(DARK_STYLESHEET)
        lo = QVBoxLayout(d)
        lo.setContentsMargins(20, 16, 20, 16)
        lo.setSpacing(6)
        lo.addWidget(QLabel("<b>Keyboard Shortcuts</b>"))
        shortcuts = [
            ("Ctrl+N", "Add new node"),
            ("Ctrl+G", "Create group"),
            ...
            ("Right-click group", "Group context menu (Set Color)"),
        ]
        for k, v in shortcuts:
            ...
            vl = QLabel(v)
            vl.setStyleSheet("color:#636e72;font-size:12px")
            row.addWidget(kl)
            row.addWidget(vl, 1)
            lo.addLayout(row)
        lo.addStretch()
